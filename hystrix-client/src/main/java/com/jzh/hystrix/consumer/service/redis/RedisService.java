package com.jzh.hystrix.consumer.service.redis;
...
public class RedisService {
    ...
    public static final String ACCESS_SYSTEMCONFIG_TOKEN = "X-Access-SystemConfig-Token";
    ...
    public Object getObjectByKey(String key, String filed){
        Object obj=null;
        if(key!=null && key!="null") {
            //开启redis，用户数据放在redis中，从redis中获取
            if(redisTemplate.opsForHash().hasKey(key,filed)){
                //redis中存在，拿出来使用
                obj=redisTemplate.opsForHash().get(key,filed);
                redisTemplate.expire(key, BusinessConstants.MAX_SESSION_IN_SECONDS, TimeUnit.SECONDS);
            }
        }
        return obj;
    }
    ...
    //    String token2 = request.getHeader(ACCESS_TOKEN);
        String token = String.valueOf(request.getSession().getAttribute(ACCESS_TOKEN));
    ...
    public void deleteObjectBySession(HttpServletRequest request, String key,String attribute){
        if(request!=null){
            String token = request.getHeader(attribute);
            if(StringUtil.isNotEmpty(token)){
                //开启redis，用户数据放在redis中，从redis中删除
                redisTemplate.opsForHash().delete(token, key);
            }
