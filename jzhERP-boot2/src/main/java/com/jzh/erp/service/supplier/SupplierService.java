...
public class SupplierService {
    ...
    public Supplier getSupplierByNameAndType(String supplier, String type) throws Exception{
        Supplier sInfo=null;
        try{
            sInfo = supplierMapperEx.getSupplierByNameAndType(supplier, type);
        }catch(Exception e){
            JzhException.readFail(logger, e);
        }
        return sInfo;
    ...
    public int batchDeleteSupplierByIds(String ids)throws Exception {
        ...
        //校验通过执行删除操作
        try{
            result = supplierMapperEx.batchDeleteSupplierByIds(new Date(),userId==null?null:Long.valueOf(userId.toString()),idArray);
        }catch(Exception e){
            JzhException.writeFail(logger, e);
        }
        return result;
