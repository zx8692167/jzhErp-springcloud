package com.jzh.erp.service.redis;
...
public class RedisService {
    ...
    public Object getObjectFromSessionByKey(HttpServletRequest request, String key){
        ...
        String token=null;
        String token2 = request.getHeader(ACCESS_TOKEN);//页面request传过来的token
        String token3 = String.valueOf(request.getSession().getAttribute(ACCESS_TOKEN)); //session会话的token
        if((token3!=null&& token3!="null")) {
            token = token3;
        }else if((token2!=null&& token2!="null")){
            token = token2;
        }
