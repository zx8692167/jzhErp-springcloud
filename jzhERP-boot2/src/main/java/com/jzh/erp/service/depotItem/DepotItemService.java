package com.jzh.erp.service.depotItem;
...
import com.jzh.erp.datasource.mappers.*;
import com.jzh.erp.datasource.vo.DepotItemStockWarningCount;
import com.jzh.erp.datasource.vo.DepotItemVo4Stock;
...
import com.jzh.erp.service.materialExtend.MaterialExtendService;
...
public class DepotItemService {
        ...
        if (null != rowArr && rowArr.size()>0) {
            ...
            for (int i = 0; i < rowArr.size(); i++) {
                ...
                if (StringUtil.isNotEmpty(depotHead.getLinkNumber())
                    ...
                    if("add".equals(actionType)) {
                        ...
                        if(depotItem.getOperNumber().add(finishNumber).compareTo(preNumber)>0) {
                            Object systemConfig = redisService.getObjectFromSessionByKey(request,"systemConfig");
                            Boolean overFlag = ((Map)systemConfig).get("overLinkBillFlag").toString()=="1"?true:false;
                ...
                if(BusinessConstants.DEPOTHEAD_TYPE_OUT.equals(depotHead.getType())){
                    ...
                    //HttpServletRequest request = ((ServletRequestAttributes) Objects.requireNonNull(RequestContextHolder.getRequestAttributes())).getRequest();
                    Object systemConfig = redisService.getObjectFromSessionByKey(request,"systemConfig");
                    Boolean minusStockFlag = ((Map)systemConfig).get("minusStockFlag").toString()=="1"?true:false;
                    if(!minusStockFlag && stock.compareTo(thisBasicNumber)<0){
    ...
    public void updateMaterialExtendPrice(Long meId, String subType, JSONObject rowObj) throws Exception {
        HttpServletRequest request = ((ServletRequestAttributes) Objects.requireNonNull(RequestContextHolder.getRequestAttributes())).getRequest();
        Object systemConfig = redisService.getObjectFromSessionByKey(request,"systemConfig");
        Boolean updateUnitFlag = ((Map)systemConfig).get("updateUnitPriceFlag").toString()=="1"?true:false;
