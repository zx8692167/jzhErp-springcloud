...
import com.jzh.erp.datasource.mappers.DepotMapper;
import com.jzh.erp.datasource.mappers.DepotMapperEx;
...
public class DepotService {
    ...
    public long insertDepot(JSONObject obj, HttpServletRequest request)throws Exception {
    ...
    public int batchDeleteDepotByIds(String ids)throws Exception {
        ...
        //校验通过执行删除操作
        try{
            result = depotMapperEx.batchDeleteDepotByIds(new Date(),userId==null?null:Long.valueOf(userId.toString()),idArray);
        }catch(Exception e){
            JzhException.writeFail(logger, e);
        }
        return result;
    ...
    public JSONArray findDepotByCurrentUser() throws Exception {
        ...
        if (null != dataList) {
            ...
            boolean depotFlag = ((Map)systemConfig).get("depotFlag").toString()=="1"?true:false;
            if(depotFlag) {
                ...
                String depotStr =((Map)basicDataConfig).get(type).toString();
    ...
    public Object parsePriceByLimit(BigDecimal price, String type, String emptyInfo, HttpServletRequest request) throws Exception {
        ...
        String priceLimit = String.valueOf(((Map)role).get("priceLimit"));
